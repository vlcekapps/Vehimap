...
- `Globální hledání`: `Ctrl+F` přesune fokus do hledání, `Ctrl+R` obnoví výsledky bez ztráty výběru a `Ctrl+O`, `Ctrl+P` nebo `Enter` na seznamu otevře vybraný výsledek
...
- `Náklady napříč vozidly`: `Ctrl+F` přesune fokus do hledání vozidel, tlačítko `Obnovit` nebo `Ctrl+R` obnoví období, `Ctrl+P` přesune fokus na rozpad nákladů vybraného vozidla, `Enter` na seznamu nebo `Ctrl+O` zobrazí detail vozidla a `Ctrl+U` nebo `F2` upraví vybrané vozidlo
