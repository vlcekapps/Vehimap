...
- `Náklady napříč vozidly`: `Ctrl+F` přesune fokus do hledání vozidel, tlačítko `Vymazat` smaže dotaz a vrátí fokus do hledání, tlačítko `Obnovit` nebo `Ctrl+R` obnoví období, `Ctrl+P` přesune fokus na rozpad nákladů vybraného vozidla, `Enter` na seznamu nebo `Ctrl+O` zobrazí detail vozidla a `Ctrl+U` nebo `F2` upraví vybrané vozidlo
