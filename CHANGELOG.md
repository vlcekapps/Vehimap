...
### Změněno
- Velikost aktualizace a průběh stahování používají jednu sdílenou formátovací službu a respektují uživatelský oddělovač desetinných míst; platformní update služba, kontrolní dialog i progress dialog už nemají tři vlastní kopie převodu B/KB/MB/GB ani hardcoded `100 %`. HTML přehled nákladů a tiskový přehled vozidel současně dokončily přechod na sdílené formátování data a času.
- Windows zástupce po startu a Linux autostart položka mají lokalizovaný EN/CS popis. Tray tooltip používá přímo název aktuálního kanálu bez pevně připojeného anglického slova `Desktop`, takže přístupné okno akcí nemusí tento text odstraňovat podle konkrétního jazyka nebo kanálu.
- Plná data v editorech historie, tankování, připomínek a údržby, v časové ose, analýze tankování, globálním hledání, nákladových obdobích, servisní knížce, automatických zálohách, aktualizačním dialogu a HTML exportu se nově zobrazují a přijímají podle aktivního jazyka (`30.04.2026` v češtině, `4/30/2026` v angličtině). Uložení zůstává kanonické a kompatibilní; změna jazyka proto data nepřepisuje a staré den-měsíc-rok hodnoty zůstávají přijímané.
- Běžné EN/CS rozhraní už nepoužívá historické vývojářské formulace jako `.NET větev`, `desktopová větev`, `původní AHK verze`, `publikovaný desktopový build` nebo `portable data`; shell, obnova zálohy, kontrola aktualizací a popis uložení dat nyní vysvětlují produktové chování běžným uživatelským jazykem. Přesný terminologický guard brání návratu těchto textů, zatímco technické názvy zůstávají dostupné v diagnostice.
- Uživatelské chyby v desktop shellu, editorech, exportech, automatických zálohách a diagnostice už nepřipojují syrové `ex.Message`; společná EN/CS služba je bezpečně třídí na přístup, chybějící soubor, souborovou operaci, síť, neplatná data, nedostupnou operaci nebo neočekávanou chybu. Interní cesty a technické texty systémových výjimek se tak nezobrazují ani nečtou odečítačem.
- Chyba načtení datové sady dál ukazuje datovou složku, cestu k SQLite databázi a doporučení k obnově, ale nezveřejňuje syrový parserový nebo systémový text; konkrétní validace chybějícího zdroje spravované přílohy zůstává lokalizovanou formulářovou chybou.
- Editorové known-value volby už neobsahují natvrdo zapsané anglické kompatibilní aliasy; kategorie, profily, paliva, doklady a opakování připomínek rozpoznávají staré EN/CS varianty výhradně přes společné resource keys a kanonické legacy hodnoty dál ukládají beze změny.
- I18n katalog má novou překladovou quality gate: každá shodná EN/CS hodnota obsahující slova musí být výslovně prověřená, jinak test odhalí pravděpodobně zkopírovaný nepřeložený text. Formátovací šablony bez slov zůstávají povolené automaticky.
...
- Dialog dokončení údržby zpřístupňuje kontext vozidla, plánu, aktuálního stavu a popisu akce jako UIA texty ještě před editačními poli.
- Samostatná workspace okna mají nově přístupné souhrnné texty pod nadpisem s unikátními window-scoped `AutomationId`, takže UIA struktura neobsahuje jen nadpis a vložený obsah.
...
- Top-level desktopová okna mají nově explicitní `CanResize` deklaraci; standardní shell, workspace, servisní knížka a editorová okna jsou hlídaná jako resizable a jen krátké transientní plochy zůstávají dokumentovanou výjimkou.
...
- Jazyk zvolený ve Windows instalátoru C# větve se po instalaci nebo aktualizaci bere jako vědomá volba uživatele a smí přepsat pouze `app/language`; jednotky, oddělovače a měna zůstávají chráněné a doplňují se jen při chybějících hodnotách. Startup lokalizace se nově nastaví před sestavením auditu, časové osy, tooltipu na liště a notifikací, takže background texty nemíchají češtinu s angličtinou.
...
- Workspace viewmodely už při každé změně root shellu neposílají plošné `PropertyChanged("")`; tím se odstraňuje stack overflow při startu nad lokalizovanými `ComboBox` volbami pro časovou osu, náklady a dashboard.
...
- Interní identifikátory chytrého poradce a analýzy tankování už nepoužívají názvy svázané s `km` nebo `litry`; unit/currency guard tak nepotřebuje výjimku pro aplikační služby `LegacySmartAdvisorService` a `LegacyFuelAnalysisService`.
- Projekční viewmodely tankování a analýzy tankování používají jednotkově neutrální názvy jako `FuelAmount`, `PricePerVolume` a `CostPerDistance`; XAML tak už nepotřebuje výjimku pro binding `Liters`, když se stejná hodnota může v UI zobrazit v litrech nebo galonech.
- Projekční viewmodel nákladů používá jednotkově neutrální `CostPerDistance` místo `CostPerKm`; kanonický výpočet nákladů za kilometr zůstává ve výpočtové vrstvě, ale XAML už zobrazuje cenu za aktuálně zvolenou vzdálenost.
- Resource klíče pro analýzu tankování a chytrého poradce byly sjednocené na jednotkově neutrální názvy jako `CostPerDistanceUnavailable`, `PricePerVolumeUnavailable` a `FuelAmountInvalid`; texty zůstávají EN/CS stejné, ale nové UI už vývojářům nenapovídá pevné km/litry.
...
- Editorová workflow vrstva používá pro vstupní hodnoty z UI jednotkově neutrální proměnné `volumeText` a `intervalDistanceText`; kanonické `liters`/`intervalKm` názvy zůstávají až za normalizací při ukládání.
- České resource texty pro diagnostiku, import legacy dat, kalendářový export a HTML servisní knížku byly sjednocené na přirozenější popisky `Běhové prostředí` a `Podrobnosti`, aby uživatelské výstupy nepřebíraly anglické `Runtime` ani technické `Detail`.
...
- I18n conformance guard nově kontroluje skutečné string literály místo celých řádků, takže zachytí uživatelské jednotkové/měnové texty bez falešných nálezů z resource klíčů nebo XAML bindingů. Servisní knížka zároveň překládá známé kategorie vozidel a typy servisních dokladů a dashboard/export nákladů normalizuje starší české i anglické stavové texty do aktuálního jazyka.
- Hlavní projekční služba a tiskový HTML přehled už nepoužívají vlastní české stavové aliasy pro prioritu termínů, filtr časové osy, `bez upozornění` nebo neaktivní vozidla; kompatibilitu řeší přes legacy konstanty a EN/CS resource hodnoty, takže tyto soubory vypadly z i18n hardcoded-Czech allowlistu.
- Desktop runtime logika už nepoužívá jednorázové české UI literály pro výpočet opakování připomínek ani rozpoznání režimu přílohy dokladu; opakování jde přes pojmenované legacy konstanty a režimy příloh přes EN/CS resource aliasy, takže další dvě výjimky vypadly z i18n hardcoded-Czech allowlistu.
- Filtry hlavního seznamu vozidel, filtr časové osy, období nákladů a řazení workspace už rozpoznávají staré uložené EN/CS labely přes sdílené resource-backed aliasy nad stabilními klíči; dotčené desktopové preference soubory tím vypadly z hardcoded-Czech allowlistu.
- Navigační cíle desktop shellu (`vehicle`, `history`, `fuel`, `record`, `maintenance`, `reminder`, `costs`) už nepoužívají české labely přímo v C# normalizaci; staré české názvy evidencí se přijímají přes EN/CS resource aliasy a `DesktopEntityKinds.cs` vypadl z hardcoded-Czech allowlistu.
- Editorové známé hodnoty v desktopu už neobsahují české legacy tokeny přímo jako string literály; `KnownValueOptions` používá `LegacyKnownValues` a EN/CS resource aliasy, takže staré hodnoty jako krátká kategorie nebo opakování zůstávají kompatibilní, ale soubor vypadl z hardcoded-Czech allowlistu.
- Rozpoznávání servisních dokladů v servisní knížce používá EN/CS resource keyword seznam místo českých slov přímo v aplikační službě; české i anglické faktury/účtenky zůstávají rozpoznané a `LegacyServiceBookService.cs` vypadl z hardcoded-Czech allowlistu.
- `LegacyKnownValueDisplayService` už neobsahuje duplicitní české mapovací hodnoty; známé legacy hodnoty porovnává přes EN/CS resource hodnoty a ASCII kompatibilní aliasy, takže další aplikační služba vypadla z hardcoded-Czech allowlistu.
- Legacy normalizátor kategorií a opakování připomínek už nevrací volné české hodnoty přímo z kódu; používá pojmenované `LegacyKnownValues` konstanty, takže `LegacyVehicleValueNormalization.cs` vypadl z hardcoded-Czech allowlistu.
- Startovací balíček pro vozidlo už pro doporučování servisních šablon, připomínek, profilu vozidla a rozhodování podle kategorie/pohonu nepoužívá české UI texty jako řídicí tokeny; české zůstávají jen katalogové/legacy šablony a mapy, zatímco runtime logika používá interní klíče a EN/CS resource aliasy.
...
- Runtime služby v aplikační, desktopové a SQLite vrstvě už při chybějícím explicitním lokalizéru nepadají zpět na češtinu; používají aktuální UI kulturu přes `ResourceAppLocalizer()`, zatímco explicitní české lokalizéry zůstávají jen pro kompatibilní aliasy nebo dvojjazyčné katalogy.
...
- Krátký status při selhání uložení volby `Zobrazovat dashboard při startu` už nezobrazuje syrový text výjimky a vrací přepínač na skutečnou uloženou hodnotu.
...
### Přidáno
- Repozitář nově deklaruje licenci `GPL-3.0-or-later` s copyright holderem Pavel Vlček; přibyly soubory `LICENSE`, `COPYING`, `COPYRIGHT-NOTICE.txt`, `THIRD-PARTY-NOTICES.md`, složka `LICENSES/`, release licensing dokumentace a compliance gate pro SPDX hlavičky i přítomnost licencí v publish výstupu.
...
- Lokalizační pilot C# shellu nově pokrývá hlavní menu `Soubor`/`Vozidlo`/`Přehledy`/`Rychlé akce`, levý panel seznamu vozidel, názvy pracovních karet, tlačítka `V okně`, vybrané shell statusy a diagnostické texty `O programu`; guard testy hlídají shodu EN/CS resource klíčů i návrat hardcoded českých textů do pilotních shell oblastí.
- Dialog `Nastavení` nově bere i přístupný název oblasti voleb z EN/CS `.resx`, takže pilotní lokalizace pokrývá nejen viditelné popisky, ale i screen-reader metadata.
- Editor vozidla nově používá EN/CS `.resx` i pro titulky dialogu, úvodní instrukce a validační hlášky ve stavovém live-regionu; anglický profil tak nedostává české chyby při ukládání vozidla.
- Lokalizační pilot nově pokrývá i statické texty dialogových editorů `Historie`, `Tankování` a `Plán údržby`, včetně field labelů, placeholderů, přístupných názvů, nápovědy `Ctrl+S`/`Escape` a jednotkových popisků tachometru, objemu paliva a servisních intervalů.
- Lokalizační pilot nově pokrývá také statické texty workspace karet `Historie`, `Tankování` a `Plán údržby`, včetně vyhledávání, řazení, seznamů, akčních tlačítek a sekcí analýzy tankování; guard testy hlídají, aby se do těchto pilotních XAML ploch nevracely hardcoded české UI texty.
- Lokalizační pilot nově pokrývá statické texty editorů a workspace karet `Připomínky` a `Doklady`, včetně vyhledávání, řazení, akčních tlačítek, popisků příloh, přístupných názvů a dokladových cest; guard testy hlídají resource bindingy i návrat hardcoded českých XAML textů.
- Lokalizační pilot nově pokrývá runtime souhrny, detailové panely, výsledky hledání a screen-reader popisky položek evidencí `Historie`, `Tankování`, `Připomínky`, `Plán údržby` a `Doklady`; při ztrátě výběru vozidla se stejné prázdné stavy berou z EN/CS `.resx` zdrojů.
- Lokalizační pilot nově pokrývá statické texty workspace `Detail vozidla`, včetně akcí nad vozidlem, souvisejících evidencí, seznamu stavů evidencí, poslední historie a přístupných `ItemType` popisků.
- Lokalizační pilot nově pokrývá statické texty workspace karet `Globální hledání` a `Časová osa`, včetně hledání, filtrů, řazení, seznamů, detailních panelů, nápověd a přístupných typů položek.
- Lokalizační pilot nově pokrývá statické texty termínových přehledů `Blížící se termíny` a `Propadlé termíny`, včetně vyhledávání, filtrů, řazení, volitelných datových problémů, detailních panelů a přístupných typů položek.
- Lokalizační pilot nově pokrývá statické texty workspace `Chytrý poradce`, včetně hledání, filtrů priority/kategorie/vozidla, akčních tlačítek, klávesové nápovědy, seznamu doporučení a detailního panelu.
- Lokalizační pilot nově pokrývá statické texty workspace `Audit dat`, včetně hledání, řazení, akčních tlačítek, klávesové nápovědy, seznamu auditních položek a přístupného typu položky.
- Lokalizační pilot nově pokrývá statické texty workspace `Náklady`, včetně období nákladů, exportních akcí, hlavních tlačítek, hledání, seznamu nákladových souhrnů a přístupného typu položky.
- Lokalizační pilot nově pokrývá také dynamické texty nákladového workflow: stavy výpočtu, popisek období, souhrn a srovnání nákladů, detail vybraného vozidla, výsledky hledání, validační hlášky období a exportní statusy používají EN/CS `.resx`; uložená preference období nově používá stabilní klíče `year_to_date` / `custom` a staré české popisky zůstávají kompatibilně přijímané.
- Lokalizační pilot nově pokrývá runtime stavové, validační a přílohové hlášky editorů `Připomínky` a `Doklady`, včetně dialogů pro výběr souboru, dostupnosti spravovaných/externích příloh a akcí otevřít soubor/složku nebo zkopírovat vyřešenou cestu.
- Lokalizační pilot nově pokrývá také runtime stavové, validační, persist a mazací hlášky editorů `Historie`, `Tankování` a `Plán údržby`, včetně dokončení údržby a potvrzení kaskádového odstranění vozidla; jednotkově citlivé texty používají aktivní jednotku nebo neutrální formulaci.
- Lokalizační pilot nově pokrývá další shell/workflow okraje: titulky samostatných workspace oken, datový režim shellu, chybu načtení poškozené databáze, automatické zálohy, stav seznamu vozidel, chyby uložení řazení/filtrů, filtrované souhrny `Audit dat` a `Chytrý poradce`, potvrzení mazání vozidla v host okně a textové a11y live-region hlášky editorových polí.
- Lokalizační pilot nově pokrývá statické texty workspace `Dashboard`, včetně hlavních akcí, volby zobrazení při startu, klávesové nápovědy, nadpisů sekcí, seznamů auditu/nákladů/termínů a přístupných typů položek.
- Lokalizační pilot nově pokrývá krátké akční stavové hlášky workspace `Dashboard`, `Plán údržby` a `Detail vozidla`, aby dokončení údržby, doporučené šablony a prázdný balíček pro vozidlo používaly EN/CS `.resx` zdroje.
- Lokalizační pilot nově pokrývá dialog `Balíček pro vozidlo` / `Doporučené servisní šablony`: titulky, nápověda, seznam, detail, tlačítka, souhrny a screen-reader popisky položek jdou přes EN/CS `.resx`.
- Katalog balíčku pro vozidlo a ručních doporučených servisních šablon nově lokalizuje názvy, kategorie, podkategorie, poznámky, profil vozidla a stavové hlášky přes EN/CS `.resx`; kontrola duplicit zároveň rozpozná staré české i nové anglické názvy šablon, aby se po změně jazyka nenabízely duplicitní úkony.
- Lokalizační pilot nově pokrývá globální přístupnou nápovědu rozbalovacích seznamů, hlášky a potvrzení rozpracovaných editorů, lokalizované popisy akcí v těchto potvrzeních a předvyplněnou GitHub issue šablonu pro zpětnou vazbu; guard testy hlídají, aby se tyto shellové texty nevracely jako hardcoded čeština.
- Lokalizační pilot nově pokrývá další app-level okraje: hlášky jednorázového jazykového seedu z instalátoru, potvrzení obnovy zálohy, krátké nativní tray menu, popisy zavření rozpracovaných evidenčních editorů v samostatných oknech a režim spuštění v diagnostice `O programu`.
- Lokalizační pilot nově pokrývá SQLite 2.0 migrační a health diagnostiku: hlášky automatické migrace, cleanupu zbylých legacy TSV/INI souborů, `PRAGMA quick_check`, schema markeru, příloh a text kopírovaný z dialogu `Kontrola datové sady 2.0` používají EN/CS `.resx`.
- Lokalizační pilot nově pokrývá také storage-level chyby balíčků vozidel a obalové chyby načítání legacy dat, takže import/export `*.vehimapvehicle` ani poškozené legacy TSV/INI soubory nemíchají český detailový rámec do anglického profilu.
- Lokalizační pilot nově pokrývá i parser a obalové chyby starších textových `.vehimapbak` záloh; SQLite 2.0 importní fallback tak při anglickém profilu nevrací české hlášky poškozené nebo nedostupné zálohy.
- Lokalizační pilot nově pokrývá platformní okrajové diagnostiky pro otevření souboru/složky a vytvoření autostartu; servisní knížka zároveň rozpozná servisně relevantní doklady podle českých i anglických klíčových slov.
- Lokalizační pilot nově pokrývá dialog `Dokončení údržby`, včetně titulku, nápovědy, field labelů, jednotkového popisku tachometru, validačních hlášek a stavů kopírování diagnostiky v dialogu `O programu`.
- Lokalizační pilot nově pokrývá také dialog `Zkontrolovat aktualizace`, platformní výsledek kontroly aktualizací a průběhové okno stahování aktualizace, včetně titulků, tlačítek, detailů pro schránku, progress statusů, validačních chyb manifestu a EN/CS textů dynamického update viewmodelu.
- Lokalizační pilot nově pokrývá nízkoúrovňové diagnostiky update manifestu a legacy TSV/attachment sekcí; chyby poškozených starších dat nebo textových záloh tak při migraci/importu používají EN/CS `.resx` místo pevných českých parser vět.
- Další i18n pass odděluje uložené legacy známé hodnoty od jejich zobrazení: kategorie vozidel, stav/pohon/profil vozidla, typ paliva, typ dokladu a opakování připomínek zůstávají v datech stabilní kvůli kompatibilitě, ale seznam vozidel, detail vozidla, tankování, doklady, připomínky, časová osa, globální hledání a exporty nákladů je zobrazují přes EN/CS `.resx`; vyhledávání zároveň indexuje původní i lokalizované hodnoty.
- Editorové ComboBoxy pro známé hodnoty teď používají oddělené `Value`/`Label`: uživatel vidí lokalizovaný EN/CS popisek, ale do SQLite se ukládá stabilní legacy token; stejné pravidlo platí i pro dialog balíčku pro vozidlo a neznámé vlastní hodnoty zůstávají beze změny.
- Unit testy mají deterministickou českou výchozí kulturu i na nečeských CI runnerech; i18n testy, které ověřují angličtinu, si kulturu nastavují explicitně.
- Lokalizační pilot nově pokrývá také krátké desktopové notifikační okno a titulek upozornění na automatickou zálohu; accessibility název, nápověda a nadpis notifikace jdou přes EN/CS `.resx` zdroje.
- Začal první domain i18n pass: analýza tankování generuje statusy, upozornění, souhrny, severity a přístupné labely přes EN/CS `.resx`; desktop projekce používá aktivní jazyk a pro anglické zobrazení formátuje čísla s anglickou desetinnou tečkou.
- Druhý domain i18n pass pokrývá auditní engine: auditní kategorie, titulky, zprávy, fallback neznámého vozidla, severity a auditní souhrn používají EN/CS `.resx`; Chytrý poradce zároveň rozpozná české i anglické auditní kategorie.
- Třetí domain i18n pass pokrývá Chytrého poradce: status, priority, kategorie, akční popisky a vlastní doporučení z auditu, tankování a nákladů používají EN/CS `.resx`; texty převzaté z časové osy zůstávají na samostatný timeline pass.
- Čtvrtý domain i18n pass pokrývá časovou osu: generované druhy položek, titulky TK/ZK/dokladů, termínové stavy, servisní stav podle kilometrů, fallbacky, litry, tachometr a základní nákladový text používají EN/CS `.resx`; Chytrý poradce zároveň rozpozná české i anglické stavové texty z časové osy.
- Pátý domain i18n pass pokrývá globální hledání: viditelné sekce, fallbacky, souhrny vozidel, tankování, dokladů, připomínek a údržby, stav příloh a detail výsledku používají EN/CS `.resx`; interní `EntityKind` hodnoty zůstávají stabilní kvůli navigaci do správné evidence.
- Šestý domain i18n pass pokrývá termínové přehledy a dashboard timeline: filtry, prázdné stavy, detaily vybrané položky, souhrny blížících se/propadlých termínů, pseudo položky chybějící zelené karty a datové nedostatky používají EN/CS `.resx` a přitom dál rozpoznávají dříve uložené české hodnoty filtrů.
- Sedmý domain i18n pass pokrývá rychlé akce a krátké workspace statusy: nejbližší TK/ZK/připomínka/servis/doklad, filtrované kontroly, otevření aktuálního upozornění, chybějící zelené karty, refresh hlášky workspace a navigační statusy Chytrého poradce používají EN/CS `.resx`; rychlé akce i background snapshot zároveň rozpoznají české i anglické `Bez upozornění` / `No alert`.
- Osmý i18n pass pokrývá aplikační workflow statusy desktop shellu: export/import záloh, balíčky vozidel, tiskový přehled, health check datové sady, datové/předmigrační složky, externí odkazy, update chyby, dialogový controller a background/tray snapshot používají EN/CS `.resx`.
- Devátý i18n pass pokrývá servisní knížku: okno, generované položky, stavové hlášky, související navigaci i HTML export používají EN/CS `.resx`; český resource soubor byl přejmenován na runtime-kompatibilní `Strings.cs-CZ.resx` a testy nově ověřují, že localizer načítá i nové české service-book klíče.
- Desátý i18n pass pokrývá ruční ICS export a dynamické texty karty `Časová osa`: názvy událostí v kalendáři, popisy položek, stavové hlášky exportu, souhrny časové osy i detail vybrané položky používají EN/CS `.resx`; uložená preference filtru časové osy nově používá stabilní hodnoty `all/future/past` a staré české/anglické popisky zůstávají kompatibilně přijímané.
- Další i18n pass pokrývá projekční souhrny seznamu vozidel, detailu vozidla a základních evidencí `Historie`, `Tankování`, `Připomínky`, `Plán údržby` a `Doklady`; fallback hodnoty, stavové texty, platnost dokladů, dostupnost příloh a poslední události používají EN/CS `.resx` místo pevných českých řetězců.
- Lokalizační dokumentace nově výslovně označuje měnu, peněžní formátování, km/l/míle/galony a exportní výstupy jako povinný i18n/unit conformance blocker před širším veřejným testováním Vehimapu 2.0.
- Lokalizační pilot nově pokrývá také globální bezpečnostní dialogy `Potvrzení` a `Kontrola datové sady 2.0`, včetně přístupných nápověd, tlačítek, diagnostických statusů a guard testů proti návratu hardcoded českých textů.
- Lokalizační pilot nově pokrývá chrome samostatných workspace oken pro detail vozidla, historii, tankování, připomínky, údržbu, doklady, audit, dashboard, náklady, hledání, termíny, časovou osu a chytrého poradce; titulky pro čtečky, help texty a zavírací akce jdou přes `.resx`.
- Lokalizační pilot nově pokrývá i přístupné okno `Akce na liště`: titul, popis, sekce, tlačítka, help texty, stav pozadí a `TrayActionsDialogViewModel` defaulty jdou přes EN/CS `.resx` a guard testy hlídají návrat hardcoded českých textů.
- Nastavení upozornění na údržbu podle vzdálenosti v C# větvi nově respektuje zvolenou jednotku vzdálenosti: v anglickém/americkém profilu lze zadat míle, v českém kilometry, ale do SQLite se dál ukládá normalizovaná kilometrová hodnota pro kompatibilní výpočty.
- Lokalizační pilot nově pokrývá i viditelné volby filtrů hlavního seznamu vozidel, výchozí prázdné texty detailu vozidla, režimy příloh dokladů a generický fallback chyby uložení; staré české i nové anglické labely filtrů/příloh zůstávají kompatibilní, aby změna jazyka nerozbila uložené preference.
- Lokalizační pilot nově pokrývá také viditelné volby řazení v evidencích, přehledech, auditu a globálním hledání; řazení používá stabilní interní klíče a přijímá staré české i nové anglické labely, takže uložené preference přežijí přepnutí jazyka.
- Editory historie, tankování, plánu údržby a dokončení údržby nově zobrazují a parsují tachometr, intervaly a objem paliva podle zvolených jednotek, ale ukládají kanonické kilometry a litry; nastavení zároveň odmítne nejednoznačnou kombinaci stejného oddělovače tisíců a desetin.
- Stavové texty plánu údržby a potvrzení dokončení servisního úkonu nově respektují zvolenou jednotku vzdálenosti i ve výstupech typu `Za 1,000 mi` nebo `Tachometr: 200 mi`, zatímco uložená hodnota zůstává kanonicky v kilometrech.
- Globální hledání nově formátuje tachometr, servisní intervaly a množství paliva podle zvolených jednotek, takže výsledky tankování a údržby v americkém profilu ukazují míle a US galony místo kanonických km/l.
- Analýza tankování nově formátuje také upozornění na klesající tachometr podle zvolené jednotky vzdálenosti, takže stejné varování v americkém profilu používá míle místo pevně vložených kilometrů.
- Texty auditu a analýzy tankování už nepoužívají zkratku `klesající tachometr`; místo toho přímo říkají, že novější hodnota tachometru je nižší než dřívější záznam.
- Servisní knížka nově formátuje aktuální tachometr, historické tachometry, servisní intervaly a stav „za/po limitu“ podle zvolené jednotky vzdálenosti; SQLite dál ukládá kanonické kilometry.
- Časová osa nově formátuje tachometr, plánovaný servisní tachometr, servisní stav podle vzdálenosti a objem tankování podle zvolených jednotek; anglický profil tak zobrazuje míle a US/imperial galony, zatímco SQLite dál ukládá kanonické kilometry a litry.
- Editor tankování nově explicitně chrání přesnost objemu paliva z čerpacích stanic: hodnoty jako `3`, `3,1` a `3,12` litru se ukládají jako kanonické desetinné litry bez zbytečných nul a analýza spotřeby/ceny s nimi počítá jako s `decimal` hodnotami.
- Nastavení nově obsahuje volbu měny (`CZK`, `USD`, `EUR`, `GBP`) a aplikační formátovací službu pro částky; změna měny zatím nepřepočítává historicky uložené hodnoty a připravuje půdu pro navazující peněžní i18n pass.
- První peněžní i18n pass napojil zvolenou měnu na dashboardové a nákladové souhrny, analýzu tankování, HTML export nákladů a servisní knížku; navazující pass teď přidal stejnou měnovou službu i do časové osy, globálního hledání a aplikačního sestavení servisní knížky.
- Exporty nákladů do TSV i HTML nově používají EN/CS resource texty, zvolenou měnu a zvolené jednotky vzdálenosti/objemu; cena za vzdálenost se v exportu přepočítá na km nebo míli podle preference, zatímco uložené částky zůstávají bez kurzové konverze.
- Dashboardové nákladové souhrny, srovnání období a analýza tankování nově používají zvolené jednotky vzdálenosti/objemu i měnu: mílový profil zobrazuje cenu za míli, palivo v galonech a spotřebu jako `mpg`, zatímco SQLite dál ukládá kanonické kilometry a litry.
- Unit conformance pass odstranil z lokalizovaných ploch pevné metrické popisky tam, kde má rozhodovat uživatelská preference: staré statické `km` klíče v nastavení a servisní knížce byly odstraněny, balíček pro vozidlo používá neutrální vzdálenostní interval a nákladové hledání/stavy ani prázdný text tankování už netvrdí pevně `km` nebo `litry`.
- Tiskový HTML přehled vozidel nově používá EN/CS `.resx` pro jazyk dokumentu, titulek, meta řádek, prázdné kategorie, sloupce tabulky a stavové zkratky; desktop shell ho vytváří s aktivním localizerem místo českého fallbacku.
