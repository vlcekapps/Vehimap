...
- Appium UI smoke spouští izolovanou kopii desktopové aplikace sám, hlavní okno dohledá buď přes nativní handle procesu, nebo přes Appium `Root` UI Automation session, a k aplikaci se potom připojí přes `appTopLevelWindow`; tím se omezuje flaky selhání WinAppDriveru při hledání čerstvě otevřeného okna podle processId.
