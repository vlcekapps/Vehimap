...
| --- | --- | --- |
| P2 | After an Appium backend disconnect, test teardown still tried to delete its isolated copy while Vehimap was running. Wait for confirmed process exit; retain the fixture when it remains or cannot be checked. Never kill an unrelated process to make the test pass. | Four pure UI configuration cases cover immediate/delayed exit, timeout and failed status probing. |
| P1 | ZIP backup/package import had no expansion/entry/memory budget, and rejected ZIPs could enter the legacy text fallback. Bound ZIP metadata before allocation, stream extraction into staging, cap legacy text, and route only identified legacy headers to that parser. Validate exports before replacing their destination. | `DataArchiveSafetyTests`, oversized ZIP/text compatibility cases; limits and compatibility in [IMPORT-SAFETY.md](IMPORT-SAFETY.md). |
| P1 | Vehicle packages copied attachments and published shell state before the caller's SQLite save. Import now holds one storage lease through copying and database commit, removes new files after normal failure/cancellation and returns only committed data. | A real failing SQLite trigger after copy, cancellation, old-file preservation, retry and database round-trip in `SqliteImportSafetyTests`. |
| P2 | While package import awaited I/O, another edit/import or tray shutdown could interrupt it. Block competing mutations, shutdown and background settings writes during import; release the UI lock on every completion path. | Delayed success/failure/cancellation in `VehiclePackageImportLifecycleTests`; live Core is a separate normal-start/shutdown check, not an import dialog test. |
...
The archive/package follow-up and its 9-10 September verification are recorded in
[IMPORT-SAFETY.md](IMPORT-SAFETY.md): 852 unit, 83 compatibility and 37 UI
configuration cases passed; live Core passed 6/6 on repeat. The first Core run's
backend disconnect is preserved separately, along with the safe-teardown correction.
The local nightly is `2.0.0-nightly.local.20260909215058`. These results narrow the
following list; they do not close it wholesale.
...
- ZIP expansion/entry-count limits and bounded legacy imports now have focused regressions; see [IMPORT-SAFETY.md](IMPORT-SAFETY.md). Extreme imported datasets still need systematic arithmetic/row-count/memory fuzzing (aggregate decimal totals and enormous date ranges); archive bounds are not a total memory guarantee.
- Package import now holds the storage lease through copy and SQLite commit, cleans newly copied files on normal failure/cancellation and only then publishes shell state. There is no second caller save. Process-kill orphan cleanup for package imports and optimistic concurrency across unrelated external database writers remain open; full-backup restore journaling does not cover package imports.
