...
| High DPI and 400% scaling | 1.4.4, 1.4.10, 2.4.11 | Top-level windows now explicitly declare resize behavior and standard shell/workspace/editor windows are guarded as resizable, but formal large-font/400% evidence is not complete. | Run manual scaling pass across shell, dashboard, audit, documents, settings and all editors. |
...
| Data-replacing actions | 3.3.4 | Backup restore, vehicle-package import, update install and other import workflows are guarded for accessible consequence text where practical, including context-specific help text for the update dialog primary action, managed-attachment conversion and starter-bundle apply actions, but the ACR needs explicit manual evidence. | Verify review text, confirmation, cancel path and status messages for restore/update/import/package/bundle actions. |
| Android TalkBack roles | 1.3.1, 4.1.2 | The read-only Android shell exposes names and actions, but on the tested Avalonia 12.0.4 path TalkBack does not consistently announce standard roles such as button. Avalonia 12.1.0 source inspection still shows no Android mapping from `AutomationControlType` to a native role. | Keep standard controls, avoid label/peer hacks, and repeat the documented role smoke on every Avalonia upgrade before making Android conformance claims. |
