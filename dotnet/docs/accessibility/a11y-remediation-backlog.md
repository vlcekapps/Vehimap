...
| Forced colors / high contrast | 1.4.3, 1.4.11, 2.4.7 | Visual contrast evidence is not complete for every state and platform theme. | Capture manual Windows forced-colors pass and fix any hidden focus/status regressions. |
...
| Data-replacing actions | 3.3.4 | Backup restore, update install and import workflows are confirmation-based, but the ACR needs explicit evidence. | Verify review text, confirmation, cancel path and status messages for restore/update/import/package actions. |
