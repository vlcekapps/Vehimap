...
| 3.3.2 Labels or Instructions | A | Supports | Editor fields have labels, accessible names and help text. | Keep static accessibility label tests green. |
...
| 3.3.4 Error Prevention | AA | Partially Supports | Backup/restore/update/destructive flows use confirmations and status text; static guards require user-language help text for restore, vehicle-package import and delete actions. | Review all data-replacing actions before ACR publication, including confirmation, cancel and status behavior. |
...
| 4.1.2 Name, Role, Value | A | Partially Supports | UI Automation names, item types/statuses, headings and live regions are guarded. | TextBox UIA fallback remains known exception. |
