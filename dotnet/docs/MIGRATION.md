...
- Spravovane prilohy se v runtime, zalohach i baliccich vozidel prijimaji jen jako relativni cesty pod `attachments/...`; storage guard odmita `..`, absolutni, drive a UNC cesty a po normalizaci overuje, ze vysledna cesta zustava uvnitr ciloveho rootu.
...
- Chybove obaly pri nacitani legacy TSV/INI, storage-level chyby exportu/importu `*.vehimapvehicle` a parser/obalove chyby starsich textovych `.vehimapbak` zaloh pouzivaji EN/CS `.resx`; nizkourovnove detaily parseru legacy TSV/attachment sekci zustavaji pripojene kvuli diagnostice konkretniho radku nebo souboru a ridi se aktivnim jazykem.
...
- Checkpoint 2026-09-09: aktualni desktopovy EN/CS rozsah je uzavreny jako lokalizacni zaklad. Novy guard kontroluje vsechny sestavene preklady, jazykove fallbacky a vsechna XAML UI; desktop publish nesmi postradat ceskou resource assembly. `Podle systemu` uz neprebira posledni rucne zvoleny jazyk a stav v kopirovane health diagnostice se lokalizuje. Nasledujici soupis zachycuje postupne dokoncene oblasti; nove funkce musi dodat oba preklady soucasne, bez dalsi storage migrace.
...
- Zname legacy hodnoty vyberovych poli se zatim v datech nemigruji: kategorie vozidel, stav/pohon/profil vozidla, typ paliva, typ dokladu a opakovani pripominek zustavaji stabilni kompatibilitni hodnoty. UI, editorove ComboBoxy, balicek pro vozidlo, casova osa, globalni hledani a nakladove exporty je pri zobrazeni prekladaji pres EN/CS `.resx`; vyhledavani bere v uvahu puvodni i lokalizovanou hodnotu.
- Pred Androidem musi zustat zelena i18n brana "anglicke UI nad ceskymi legacy daty": projekce, seznamy a pristupne labely nesmi v anglickem profilu ukazovat ceske systemove texty, zatimco uzivatelska data se nesmi prekladat ani prepisovat. Staticky guard povoluje cestinu v produkcnim kodu jen v legacy token/alias hranach, parser/search klicich a sablonovych datech.
- Po zelene i18n brane vznikl Android zaklad: samostatny `net10.0-android` host a sdileny `Vehimap.Mobile` shell nad SQLite, stejnymi EN/CS resources a known-value projekcemi. Mobil neprebira ctrnact desktopovych karet; pouziva ctyri cile `Domu`, `Vozidla`, `Upozorneni`, `Vice`. Centrum vybraneho vozidla ma funkcni zanorene read-only routes pro historii, tankovani, doklady a pripominky se samostatnym seznamem a detailem polozky; zobrazeni respektuje jednotky, oddelovace a menu bez zmeny kanonickych dat, doklady skryvaji interni managed cestu a pripominky pocitaji lokalizovany stav terminu. Systemove Zpet se zpracovava pres Avalonia `TopLevel.BackRequested`, takze tlacitkova i gestova navigace postupne zavira detail polozky, seznam evidence, centrum vozidla a seznam vozidel a az z korenoveho `Domu` aplikaci. Android ma vlastni nightly AppId a app-private data; editace, synchronizace a verejna mobilni distribuce zatim nejsou soucasti tohoto checkpointu.
...
- Pred Androidem je blokujici prubezny i18n/unit/currency conformance pass pro plosne pouziti zvolene meny, formatovani peneznich castek, exportni vystupy a km/l/mile/galon texty. Dashboardove/nakladove souhrny, analyza tankovani vcetne upozorneni na klesajici tachometr, servisni knizka vcetne tachometru a servisnich vzdalenosti, casova osa vcetne tachometru/servisnich vzdalenosti/objemu paliva, globalni hledani vcetne viditelnych tachometru, servisnich intervalu a objemu paliva, stavove texty udrzby, nastaveni vzdalenostnich upozorneni, balicek pro vozidlo, nakladove hledani/stavy, nakladove TSV/HTML exporty a tiskovy HTML prehled vozidel uz zvolenou menu a jednotky respektuji nebo pouzivaji jednotkove neutralni formulaci; analyza tankovani zobrazuje pri kombinaci mile + galony spotrebu jako `mpg`. Staticky guard odmita nove pevne jednotkove/menove literaly v produkcnim UI/projekcnim/reportovacim kodu mimo storage defaulty, parser hranice a sdilene formatovaci sluzby. Preklad resource retezce sam o sobe nestaci, pokud hodnota porad obchazi formatovaci, menovou nebo jednotkovou sluzbu.
...
- Windows hardening wrapper `dotnet/build/Test-DotnetWindowsHardening.ps1`, ktery pred beta kanalem spoji release train status, historickou migracni paritu, cele `dotnet test`, nightly readiness a volitelnou kontrolu publikovane nightly; plny lokalni install smoke blokuje bez explicitniho `-AllowLocalInstallSmoke`, aby neprepsal uninstall registr existujici instalace stejneho kanalu
- migracni parity report `dotnet/build/Get-DotnetMigrationParity.ps1`, ktery mapuje historicke `src/lib/*.ahk` moduly na konkretni .NET evidence soubory; zustava auditnim dukazem pokryti, ale uz neni vazany na samostatnou AHK retirement gate ani na aktualni stable manifest
- AHK aplikace, runtime knihovny a puvodni testy jsou odstranene; read-only `Vehimap.Storage.Legacy` zustava pouze pro jednorazovou migraci dat 1.x a import starsich zaloh
...
- vsechny release kanaly overuji manifest pres `Test-DotnetPublishedRelease.ps1`; stable navic udrzuje preview alias pro uz vydane starsi buildy
...
1. Udrzet Windows stable kanal jako baseline: po kazde release/tooling zmene spustit `Test-DotnetPublishedStable.ps1 -RuntimeIdentifier win-x64 -SkipNetwork`, aby stable manifest, asset i preview alias zustaly v zelenem stavu.
2. Radu 2.0 zatim drzet jako dlouhou nightly etapu, ne jako beta kandidata. Dalsi bezny vyvoj delat pres `nightly` na `main`; Windows rucne testovat pres `dotnet/artifacts/nightly/win-x64/app/Vehimap.exe` a celou desktop matici vytvaret pres `Test-DotnetNightlyMatrix.ps1` do `dotnet/artifacts/nightly/<rid>/`. Distribuovany soubor se na Linuxu/macOS jmenuje `Vehimap`; `Vehimap.Desktop` zustava jen nazvem projektu a namespace.
...
6. Lokalizacni zaklad pred Androidem drzet jako sdilenou infrastrukturu: Windows instalator predava vybrany jazyk pres jednorazovy `installer-preferences.json`, aplikace doplni chybejici locale/jednotkove volby do SQLite a aktivni runtime uz nepouziva `settings.ini`.
   Pocetni souhrny, audit, poradce, prehledy, oznameni a terminove stavy musi pro EN/CS prochazet sdilenou pluralizacni sluzbou a resource rodinami `.One/.Few/.Other`; slozene souhrny servisni knizky, detailu vozidla, casove osy, tray tooltipu, exportu a health diagnostiky sklonuji kazdy pocet samostatne. Detail vozidla stejne zpracovava pocty pripominek, chybejicich/nedostupnych priloh a celkovych/aktivnich/pozastavenych servisnich planu. Stejne pravidlo plati pro pocty priloh v zalohach/obnove/baliccich, automaticke zalohy, dopady mazani vozidla, rychle kontrolni akce a docasna TextBox UIA oznameni. Textovy fallback navic pocita Unicode textove prvky, ne jednotlive UTF-16 code units. Viditelne labely systemovych file pickeru a dialogu O programu musi stejne jako bezne UI pochazet z resources; invariantni zustavaji jen URL, pripony, wildcard patterny a stabilni tokeny. Ulozene pocty ani SQLite schema se tim nemeni.
   Pojmenovane Windows workspaces a domenove projekce tvori dokonceny EN/CS baseline aktualnich funkci. Lokalizacni gate dale zustava povinnou regresni ochranou: testuje i sestavene resources, neutralni cestinu, anglicky fallback a navrat na systemovy jazyk; publish vyzaduje oba jazykove soubory. Stejne resource a formatovaci sluzby se sestavuji i do Linux/macOS nightly balicku. To nenahrazuje rucni accessibility validaci ani neznamena verejne vydani 2.0.
7. Desktop publish matici drzet pruchodnou pro `win-x64`, `linux-x64`, `osx-x64` a `osx-arm64`. `Test-DotnetNightlyMatrix.ps1` overuje lokalni cross-publish a strukturu archivu; verejne CI navic na nativnim runneru hlida executable bit. To jeste nenahrazuje rucni VoiceOver/Orca ani distribucni akceptaci dane platformy.
8. Android zaklad dale rozsirovat z mobilniho shellu: nejdrive read-only evidence pod vehicle hubem, nativni test dat, lokalizace a accessibility na fyzickem zarizeni, potom bezpecny prenos/synchronizace dat a CRUD. Vsechny mobilni editory budou samostatne celostranske/modalni routes; inline editor je zakazany.
9. Po Android zakladu stabilizovat macOS desktop, hlavne VoiceOver, notarizaci, app bundle a rucni update tok.
10. Linux brat jako posledni platformu; az po macOS doresit distribuci, X11/Wayland pristupnost a Orca smoke.
