...
- Po zelene i18n brane vznikl Android zaklad: samostatny `net10.0-android` host a sdileny `Vehimap.Mobile` shell nad SQLite, stejnymi EN/CS resources a known-value projekcemi. Mobil neprebira ctrnact desktopovych karet; pouziva ctyri cile `Domu`, `Vozidla`, `Upozorneni`, `Vice`. Centrum vybraneho vozidla uz ma prvni funkcni zanorene read-only routes pro historii a tankovani se samostatnym seznamem a detailem polozky; zobrazeni respektuje jednotky, oddelovace a menu bez zmeny kanonickych dat. Systemove Zpet se zpracovava pres Avalonia `TopLevel.BackRequested`, takze tlacitkova i gestova navigace postupne zavira detail polozky, seznam evidence, centrum vozidla a seznam vozidel a az z korenoveho `Domu` aplikaci. Android ma vlastni nightly AppId a app-private data; editace, synchronizace a verejna mobilni distribuce zatim nejsou soucasti tohoto checkpointu.
