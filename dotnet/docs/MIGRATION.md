...
- Po zelene i18n brane vznikl prvni Android zaklad: samostatny `net10.0-android` host a sdileny `Vehimap.Mobile` read-only shell nad SQLite, stejnymi EN/CS resources a known-value projekcemi. Android ma vlastni nightly AppId a app-private data; editace, synchronizace a verejna mobilni distribuce zatim nejsou soucasti tohoto checkpointu.
...
8. Android zaklad dale rozsirovat z hotoveho read-only shellu: nejdrive nativni test dat, lokalizace a accessibility na fyzickem zarizeni, potom navrh bezpecneho prenosu/synchronizace dat a az pote editace.
