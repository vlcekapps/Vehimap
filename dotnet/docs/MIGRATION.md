...
6. Lokalizacni zaklad pred Androidem drzet jako sdilenou infrastrukturu: Windows instalator predava vybrany jazyk pres jednorazovy `installer-preferences.json`, aplikace doplni chybejici locale/jednotkove volby do SQLite a aktivni runtime uz nepouziva `settings.ini`.
   Pocetni souhrny, audit, poradce, prehledy, oznameni a terminove stavy musi pro EN/CS prochazet sdilenou pluralizacni sluzbou a resource rodinami `.One/.Few/.Other`; slozene souhrny servisni knizky, detailu vozidla, tray tooltipu, exportu a health diagnostiky sklonuji kazdy pocet samostatne. Stejne pravidlo plati pro pocty priloh v zalohach/obnove/baliccich, automaticke zalohy, dopady mazani vozidla a rychle kontrolni akce. Ulozene pocty ani SQLite schema se tim nemeni.
