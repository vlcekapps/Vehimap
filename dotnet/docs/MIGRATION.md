...
- Pilotni oblasti jsou `Nastaveni`, `O programu` vcetne kopirovacich statusu, `Editor vozidla`, hlavni shell, hlavni menu, levy panel seznamu vozidel, nazvy pracovnich karet, staticke povrchy editoru historie/tankovani/udrzby/pripominek/dokladu, dialog `Dokonceni udrzby`, workspace `Detail vozidla`, `Globalni hledani`, `Casova osa`, `Blizici se terminy`, `Propadle terminy` a `Chytry poradce`, staticke texty jejich workspace karet vcetne vyhledavani, razeni, akcnich tlacitek, terminovych detailu, doporucovacich filtru a dokladovych priloh, globalni dialogy `Potvrzeni` / `Kontrola datove sady 2.0`, chrome samostatnych workspace oken a pristupne okno `Akce na liste`.
