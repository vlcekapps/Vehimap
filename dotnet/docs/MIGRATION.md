...
- Pilotni oblasti jsou `Nastaveni`, `O programu`, `Editor vozidla`, hlavni shell, hlavni menu, levy panel seznamu vozidel, nazvy pracovnich karet, staticke povrchy editoru historie/tankovani/udrzby/pripominek/dokladu, staticke texty jejich workspace karet vcetne vyhledavani, razeni, akcnich tlacitek a dokladovych priloh a globalni dialogy `Potvrzeni` / `Kontrola datove sady 2.0`.
