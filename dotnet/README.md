...
- pouzit nakladovy workspace s rychlym hledanim vozidel, tlacitky `Vymazat` a `Obnovit` a zkratkami `Ctrl+F` pro hledani, `Ctrl+R` pro obnovu prehledu, `Ctrl+P` pro precteni rozpadu nakladu, `Ctrl+O` nebo `Enter` pro otevreni vozidla a `Ctrl+U` / `F2` pro upravu vozidla
...
- zamknout hlavni seznam vozidel a jeho filtry po dobu aktivni editace, aby se nedalo omylem prepnout na jine vozidlo pred ulozenim nebo zrusenim editoru
