...
- spoustet Windows Appium smoke i v CI nad publish buildem desktop preview; CI smoke overuje app-level menu `Soubor`, menu `Vozidlo`, menu `Rychle akce`, jejich aktualni action-state, otevreni a zavreni pristupnych tray akci z menu `Aplikace`, ulozeni voleb automatickych zaloh z dialogu `Nastaveni` vcetne okamziteho vytvoreni `.vehimapbak` a post-create i rucni `Balicek pro vozidlo`, zatimco rozsirena sada nad izolovanou portable kopii testuje doporucene servisni sablony, potvrzeni `Splneno` z `Planu udrzby` i dashboardove akce `Dokoncit servis`, ulozeni volby Dashboardu pri startu vcetne validacni chyby v `Nastaveni`, kopirovani diagnostiky z `O programu`, detailu kontroly aktualizaci i vyresene cesty spravovane prilohy dokladu do systemove schranky a dalsi realne vytvoreni okamzite automaticke zalohy
