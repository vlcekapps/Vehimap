...
- Android má samostatný `net10.0-android` host a sdílený `Vehimap.Mobile` shell se čtyřmi cíli `Domů`, `Vozidla`, `Upozornění`, `Více`; centrum vozidla už otevírá samostatné read-only seznamy a detaily historie, tankování, dokladů a připomínek. Systémové Zpět nejprve prochází mobilní navigací přes Avalonia `TopLevel.BackRequested` a teprve z kořenového `Domů` opustí aplikaci. Nejde o desktopový RID ani o veřejný mobilní release. Mobilní informační architektura a zákaz inline editorů jsou závazně popsány v [docs/MOBILE.md](docs/MOBILE.md)
- Mobilní i desktopové produktové plochy používají stručný text orientovaný na obsah a stav; návody k samozřejmému ovládání standardních prvků patří do budoucí uživatelské příručky, ne do trvale viditelného UI ani opakovaně čteného accessibility `HelpText`.
...
- EN/CS lokalizace pokrývá současný desktopový shell, workspace, editory, generované stavy a validace, aktualizace, notifikace, přílohy, SQLite diagnostiku a reporty/exporty. Podrobný soupis dokončených oblastí a pravidla pro další vývoj jsou v [docs/I18N.md](docs/I18N.md); překlad nového UI se už neodkládá na samostatný pilot.
...
- pocetni vety pouzivaji `IAppPluralizationService`: resource rodina ma varianty `.One`, `.Few` a `.Other`, anglictina vybira jednotne/mnozne cislo a cestina spravny tvar pro `1`, `2-4` a ostatni pocty; brana pokryva hlavni evidence, audit, poradce, terminove prehledy, analyzu tankovani, naklady, detail vozidla vcetne pripominek/priloh/servisnich planu, casovou osu, startovaci balicek, servisni knizku, export kalendare, tiskovy report, health diagnostiku, tray tooltip, zalohy/obnovu, balicky vozidel, automaticke zalohy, potvrzeni mazani a titulky oznameni. Vety s vice pocty skladaji samostatne pluralizovane fragmenty a primy `LF/Format` nad migrovanou univerzalni sablonou blokuje test.
...
- shell pred kazdym prepocitem projekci znovu aplikuje jazyk z datove sady, aby programove skladane texty detailu vozidla a souhrnu nemichaly cestinu s anglictinou po startu, update nebo reloadu
- plna data v editorech, seznamech, casove ose, analyzach, hledani, nakladovych obdobich, servisni knizce, update dialogu a HTML exportu pouzivaji sdilenou culture-aware sluzbu; ceske UI zobrazuje napriklad `30.04.2026`, anglicke `4/30/2026`, ale ulozena hodnota zustava kanonicka a zmena jazyka ji nemigruje
- velikosti souboru v kontrole aktualizace, platformni update zprave a prubehu stahovani pouzivaji `IAppFileSizeFormatService`; `KB/MB/GB` se tak pocitaji na jednom miste a cislo respektuje zvolene oddelovace, stejne jako ostatni uzivatelske hodnoty
- Windows/Linux autostart popis je lokalizovany pres `.resx` a tray tooltip nepripojuje k nazvu kanalu pevny anglicky text
- staticke texty v editorech a workspace kartach historie, tankovani, planu udrzby, pripominek a dokladu jsou pilotne lokalizovane vcetne pristupnych nazvu; runtime souhrny, detailove panely, vysledky hledani a screen-reader popisky polozek techto evidenci uz take pouzivaji `.resx`; stejne tak projekcni souhrny seznamu vozidel, detailu vozidla, stavu vozidla, platnosti dokladu a dostupnosti priloh; runtime statusy, validace, persist hlasky, dialogy vyberu souboru a vysledky akci nad prilohami jsou lokalizovane pro hlavni evidencni editory a mazani vozidla; lokalizovane jsou i workspace `Detail vozidla`, `Globalni hledani`, `Casova osa`, `Blizici se terminy`, `Propadle terminy`, `Chytry poradce`, `Audit dat`, `Naklady`, `Dashboard`, servisni knizka vcetne generovaneho HTML exportu, tiskovy HTML prehled vozidel, rucni ICS export vcetne nazvu udalosti a popisu v kalendari, kratke akcni workspace statusy dashboardu/udrzby/detailu vozidla, dialog dokonceni udrzby, potvrzovaci dialog, dialog kontroly datove sady 2.0, dialog kontroly/stahovani aktualizaci i platformni update hlasky, kratke desktopove notifikacni okno, titulky/help/zavirani samostatnych workspace oken, automaticke zalohy, filtrovane souhrny auditu/poradce, a11y text-edit live-region, pristupne okno `Akce na liste`, kopirovaci statusy a runtime rezim v dialogu `O programu`, domain i18n pass analyzy tankovani, auditu dat, Chytreho poradce, casove osy, globalniho hledani, terminovych prehledu/dashboard timeline, rychlych akci, app-shell workflow statusu a nakladoveho workflow; zbyvajici dynamicke workflow statusy a domenove zpravy techto evidenci prijdou v dalsich passech
- zname legacy hodnoty vyberovych poli se neprepisuji v databazi, ale zobrazuji se lokalizovane pres `.resx`; pokryte jsou kategorie vozidel, stav/pohon/profil vozidla, typ paliva, typ dokladu a opakovani pripominek v seznamu/detailu vozidla, editorech, balicku pro vozidlo, tankovani, dokladech, pripominkach, casove ose, globalnim hledani a exportech nakladu; sdilena display sluzba a startovaci balicek rozpoznavaji stare EN/CS hodnoty pres resource hodnoty a aliasy, ne pres duplicitni ceske mapy v aplikacnim kodu
- kompatibilni aliasy patri na hranice legacy/resource vrstev: opakovani pripominek pouziva pojmenovane legacy konstanty, editorove zname hodnoty berou ulozene tokeny z `LegacyKnownValues` a stare zkratky pres EN/CS resource aliasy, a rezimy priloh dokladu se rozpoznavaji pres EN/CS resource hodnoty, ne pres nove hardcoded UI texty v desktop logice
- `KnownValueOptions` ani aplikacni display sluzba nesmi mit vlastni anglicke nebo ceske aliasy; kratke kategorie, ASCII varianty a starsi slovni tvary opakovani jsou soucasti obou resource katalogu a nezname uzivatelske hodnoty zustavaji nedotcene
...
- uzivatelske chybove statusy nesmi vypisovat `ex.Message`; `UserFacingExceptionMessageService` prevadi systemove chyby na bezpecne EN/CS kategorie, zatimco presne parserove detaily zustavaji jen na interni diagnosticke hranici
- bezne shell, update a backup texty popisuji vydani Vehimapu, dopad akce a rezim ulozeni dat bez historickych pojmu `.NET vetev`, `desktopova vetev`, `puvodni AHK verze`, `publikovany desktopovy build` nebo smisenych `portable data`; framework a storage nazvy zustavaji jen v explicitni diagnostice a migracnich detailech
...
- app-level okraje jako installer locale seed statusy, potvrzeni obnovy zalohy, kratke native tray menu, close-action texty samostatnych evidencnich oken, platformni vysledky kontroly aktualizaci, validacni chyby update manifestu a parserove chyby legacy TSV/attachment sekci uz take pouzivaji EN/CS `.resx`; startup background/tray/notifikacni texty se sestavuji az po nacteni jazyka z datove sady, aby se po update nemichala cestina s anglictinou
- platformni diagnostika pro otevreni souboru/slozky a vytvoreni autostartu uz take pouziva live EN/CS localizer; servisni knizka pri vyberu relevantnich dokladu rozpoznava ceska i anglicka servisni klicova slova z resource seznamu, ne z hardcoded UI textu v aplikacni sluzbe
...
- mena ma prvni podporovanou volbu v nastaveni (`CZK`, `USD`, `EUR`, `GBP`) a spolecnou formatovaci sluzbu; zmena meny meni zobrazeni a exportni popisky, ale bez kurzove politiky neprepocitava ani neprepisuje historicke castky
...
- vstupni hodnoty v editorove workflow vrstve pouzivaji neutralni nazvy jako `volumeText` a `intervalDistanceText`; kanonicke `liters`/`intervalKm` promenne vznikaji az po validaci a prevodu
...
- v editorech chranit standardni textovou navigaci sipkami, Home/End, Backspace/Delete a beznymi Ctrl zkratkami, rozbalit ComboBox sipkou nahoru/dolu bez nutnosti `Alt+Down`, oznamovat kurzorovy kontext pres docasny TextBox live-region fallback a po ulozeni nebo zruseni editace vozidla vratit fokus na prvni logickou akci detailu `Upravit vozidlo`
...
- otevrit pristupne `Akce Vehimapu na liste` z menu `Aplikace` nebo zkratkou `Ctrl+Shift+Y`, aby screen-reader-first cesta nezacinala v nativnim kontextovem menu oznamovaci oblasti, ktere nemusi pri otevreni spolehlive ohlasit fokus
...
- otevrit z pristupneho tray akcniho okna hlavni okno, dashboard, blizici se terminy, propadle terminy, nejblizsi TK/ZK/pripominku/servis/doklad, filtrovanou kontrolu techto oblasti, tiskovy prehled, export/import zalohy, okamzitou automatickou zalohu, slozku automatickych zaloh, export kalendare, znovunacteni dat, nastaveni, O programu, kontrolu aktualizaci nebo ukoncit aplikaci; nativni tray menu zustava jen kratka pohodlna nabidka `Zobrazit Vehimap`, `Otevrit dashboard`, `Ukoncit Vehimap`, zatimco pristupna cesta vede pres `Aplikace -> Akce na liste` nebo `Ctrl+Shift+Y`
...
- kontrolovat historickou migracni paritu AHK modulu pres `dotnet\build\Get-DotnetMigrationParity.ps1`, aby kazdy odstraneny legacy modul mel pojmenovanou .NET evidence vrstvu; nejde uz o release blocker vazany na aktualni stable manifest
...
- pred commitem vygenerovanych manifestu overit vsechny runtime manifesty pres `Test-DotnetPublishedRelease.ps1 -RuntimeIdentifier <rid> -Channel <kanal> -SkipNetwork`; stable navic hlida prechodovy preview alias
...
- na Windows runneru spustit Appium smoke nad publish buildem desktop release vcetne kontroly app-level menu a dostupnosti rychlych akci; CI instaluje WinAppDriver z pevneho MSI assetu se SHA-256 kontrolou misto anonymniho Appium `install-wad` GitHub API dotazu a UI testy spousti izolovanou aplikaci pres WinAppDriver s pracovnim adresarem testovaci kopie; pokud primarni attach selze, fallback dohleda okno pres Appium `Root` UI Automation session a pripoji se pres `appTopLevelWindow`
...
Windows instalator pro C# vetev nabizi anglictinu a cestinu. Zvoleny jazyk se do aplikace predava jednorazovym souborem `installer-preferences.json` v datove slozce kanalu, nikoli pres legacy `settings.ini`. Pri startu Vehimap bere jazyk instalatoru jako vedomou volbu uzivatele a smi podle nej prepsat pouze `app/language`; oddelovace, jednotky a mena se doplni jen tehdy, kdyz jeste chybi. Cestina jako vychozi profil pouzije kilometry, litry, desetinou carku, zadny oddelovac tisicu a `CZK`; anglictina pouzije mile, US galony, desetinnou tecku, carku pro tisice a `USD`. Pokud uzivatel tyto formatovaci preference pozdeji zmeni, instalator je pri update neprepisuje. Zmena jazyka resetuje historii desktopovych notifikaci pro dany den, aby se po update mohlo zobrazit lokalizovane upozorneni znovu.
...
Release train status nic netaguje ani nepublikuje. Jen shrne lokalni artefakty `nightly`, `beta` a `stable`, existenci manifestu v `update/`, dostupnost release skriptu, remote tagy a na konci vypise dalsi doporuceny krok. Kanalove wrappery `Test-DotnetPublishedNightly.ps1`, `Test-DotnetPublishedBeta.ps1` a `Test-DotnetPublishedStable.ps1` jen volaji spolecny `Test-DotnetPublishedRelease.ps1` se spravnym kanalem, aby pri rucni kontrole neslo omylem overovat spatny manifest. Nightly wrapper kontroluje `latest-dotnet-nightly-win-x64.ini`, rolling tag `dotnet-nightly`, prerelease verzi, release notes URL, dostupnost assetu, SHA-256, velikost assetu a `channel=nightly`. Stable wrapper navic zkontroluje prechodovy `latest-dotnet-preview-win-x64.ini`. Pokud je potreba jen offline kontrola po commitu manifestu, pridejte `-SkipNetwork`.

AHK aplikace, runtime knihovny a puvodni testy jsou z repozitare odstranene, proto uz release tok nema samostatnou AHK retirement gate. `Get-DotnetMigrationParity.ps1` zustava jako historicky auditni dukaz pokryti a `Vehimap.Storage.Legacy` jako read-only vstup pro migraci dat 1.x a import starych zaloh.
