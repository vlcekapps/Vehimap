...
- lokalizacni zaklad se zavadi pred dalsimi velkymi funkcemi a pred Android UI; pilot uz pokryva nastaveni, O programu, editor vozidla, hlavni shell, hlavni menu, levy panel seznamu vozidel, nazvy pracovnich karet, staticke povrchy editoru historie/tankovani/udrzby/pripominek/dokladu, staticke texty jejich workspace karet, globalni bezpecnostni dialogy, chrome samostatnych workspace oken a pristupne okno `Akce na liste` pres `.resx` zdroje
...
- staticke texty v editorech a workspace kartach historie, tankovani, planu udrzby, pripominek a dokladu jsou pilotne lokalizovane vcetne pristupnych nazvu; lokalizovane jsou i potvrzovaci dialog, dialog kontroly datove sady 2.0, titulky/help/zavirani samostatnych workspace oken a pristupne okno `Akce na liste`; dynamicke workflow statusy a domenove zpravy techto evidenci prijdou v dalsich passech
