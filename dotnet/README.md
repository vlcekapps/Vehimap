...
- lokalizacni zaklad se zavadi pred dalsimi velkymi funkcemi a pred Android UI; pilot uz pokryva nastaveni, O programu vcetne kopirovacich statusu, editor vozidla, hlavni shell, hlavni menu, levy panel seznamu vozidel, nazvy pracovnich karet, staticke povrchy editoru historie/tankovani/udrzby/pripominek/dokladu, dialog dokonceni udrzby, aktualizacni dialogy, staticke texty jejich workspace karet, workspace `Detail vozidla`, `Globalni hledani`, `Casova osa`, `Blizici se terminy`, `Propadle terminy`, `Chytry poradce`, `Audit dat`, `Naklady`, `Dashboard`, kratke akcni workspace statusy dashboardu/udrzby/detailu vozidla, globalni bezpecnostni dialogy, chrome samostatnych workspace oken a pristupne okno `Akce na liste` pres `.resx` zdroje
