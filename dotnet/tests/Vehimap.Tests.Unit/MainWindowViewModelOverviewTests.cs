...
public sealed class MainWindowViewModelOverviewTests
{
    ...
    public void Refresh_overview_commands_preserve_selection_and_request_list_focus()
    {
        ...
        Assert.Equal(selectedUpcoming.EntryId, viewModel.UpcomingOverviewWorkspace.SelectedUpcomingOverviewItem?.EntryId);
        ...
        Assert.Equal(selectedOverdue.EntryId, viewModel.OverdueOverviewWorkspace.SelectedOverdueOverviewItem?.EntryId);
    ...
    public void Upcoming_overview_can_include_missing_green_cards_and_audit_data_issues()
    {
        ...
        Assert.Contains("Z toho datových nedostatků:", viewModel.UpcomingOverviewWorkspace.UpcomingOverviewSummary, StringComparison.CurrentCulture);
        Assert.Contains(viewModel.UpcomingOverviewWorkspace.UpcomingOverviewItems, item => item.Kind == "data_issue" && item.Title.StartsWith("Doplňte přesné datum:", StringComparison.Ordinal));
    }
