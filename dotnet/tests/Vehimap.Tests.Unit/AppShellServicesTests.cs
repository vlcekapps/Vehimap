...
public sealed class AppShellServicesTests : IDisposable
{
    ...
    public void About_dialog_summary_exposes_author_and_channel_version()
    {
        ...
        Assert.Equal("od Vlcek apps", model.Author);
        ...
        Assert.Contains("Autor: od Vlcek apps", model.DiagnosticText, StringComparison.Ordinal);
    ...
    [Fact]
    public void About_dialog_author_credit_uses_requested_language()
    {
        var model = new AboutDialogViewModel(
            "Vehimap",
            "2.0.0",
        ...
        Assert.Contains("Author: by Vlcek apps", model.DiagnosticText, StringComparison.Ordinal);
    }
    ...
    public void Supported_settings_service_preserves_unrelated_sections_and_keys()
    {
        ...
        settings.SetValue("app", "currency", "CZK");
        ...
        service.Apply(settings, new DesktopSupportedSettingsSnapshot(45, 20, 7, 750, false, true, true, true, 3, 21, "en-US", "comma", "dot", "mi", "us_gal", "USD"));
