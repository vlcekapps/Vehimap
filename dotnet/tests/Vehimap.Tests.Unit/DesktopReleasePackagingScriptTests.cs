...
public sealed class DesktopReleasePackagingScriptTests : IDisposable
{
    ...
    public async Task Inno_template_closes_running_applications_and_offers_postinstall_launch()
    {
        ...
        Assert.Contains("UsePreviousLanguage=yes", template, StringComparison.Ordinal);
        Assert.Contains("ShowLanguageDialog=auto", template, StringComparison.Ordinal);
        ...
        Assert.Contains("{{APP_ID_UNINSTALL_KEY}}", template, StringComparison.Ordinal);
        ...
        Assert.Contains("function IsUpgrade(): Boolean", template, StringComparison.Ordinal);
        Assert.Contains("function IsFreshInstall(): Boolean", template, StringComparison.Ordinal);
        Assert.Contains("function ShouldSkipPage(PageID: Integer): Boolean", template, StringComparison.Ordinal);
        Assert.Contains("PageID = wpSelectDir", template, StringComparison.Ordinal);
        Assert.Contains("PageID = wpSelectProgramGroup", template, StringComparison.Ordinal);
        Assert.Contains("PageID = wpSelectTasks", template, StringComparison.Ordinal);
        Assert.Contains("PageID = wpReady", template, StringComparison.Ordinal);
        Assert.DoesNotContain("PageID = wpLicense", template, StringComparison.Ordinal);
        Assert.Contains("Name: \"desktopicon\"; Description: \"{cm:DesktopIconTask}\"; GroupDescription: \"{cm:ShortcutsGroup}\"; Flags: unchecked; Check: IsFreshInstall", template, StringComparison.Ordinal);
        Assert.Contains("Tasks: desktopicon; Check: IsFreshInstall", template, StringComparison.Ordinal);
    ...
    public async Task Windows_packaging_stages_root_favicon_for_installer_shortcuts()
    {
        ...
        Assert.Contains("Get-InnoUninstallAppId", script, StringComparison.Ordinal);
        Assert.Contains("APP_ID_UNINSTALL_KEY", script, StringComparison.Ordinal);
